...
static const uint32_t kPersistMagic = 0x4B49444BUL; // 'KIDK'
...
void setup()
{
    ...
    if (clockValid && scheduleValid)
    {
        ...
        time_t fireAt = 0;
        const uint32_t seconds = secondsUntilNextWake(schedule, now, &fireAt);
        if (seconds > 0)
        {
            struct tm local;
            localtime_r(&fireAt, &local);
            KIDINK_LOGF("schedule: next wake %04d-%02d-%02d %02d:%02d local",
    ...
    // No usable clock (first boot with the network down) or an unusable
    // schedule: sleep on the timer alone and try again shortly.
    KIDINK_LOGF("schedule: no clock or schedule, falling back to %ds",
