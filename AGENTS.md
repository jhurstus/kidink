...
- **Tests:** pytest. Test files live side-by-side with the code they test (e.g. `app/test_index.py` next to `app/__init__.py`), named `test_*.py`. Add or update tests alongside behavior changes.
  ...
  - **Hermetic config (no `config.toml`, no real secrets).** The suite must pass from a clean checkout with no `server/config.toml`. `server/conftest.py` has an autouse fixture that injects fake `KIDINK_*` settings (four required secrets + two kids) and points the TOML source at a nonexistent path, so a developer's local `config.toml` is ignored during tests. `check.sh` additionally relocates any local `config.toml` for the pytest run as a backstop. Don't reintroduce a hard dependency on a present `config.toml`; build settings through `get_settings()` / `create_app()` (or `test_config.py`'s `_settings()` helper), never by reading the file.
