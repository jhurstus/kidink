...
# --- Outfit selection (§ Weather: PoP ≥ 25% → rain, else by the day's high) ---
...
def test_outfit_cutoffs(high: float, precip: int, outfit: Outfit) -> None:
    panel = build_weather(_TARGET, _day(high=high, precip=precip))
...
def test_override_high_replaces_the_high_only() -> None:
    day = override_high(_day(high=55.0, precip=30), 95)
...
def test_bar_and_outfit_both_read_the_same_high() -> None:
    # § Weather: two systems sharing one input — the bar points at, and the
    # clothing cutoffs read, the same actual high.
    panel = build_weather(_TARGET, _day(high=51.8))
...
def test_arrow_position_at_band_edges(high: float, percent: float) -> None:
    panel = build_weather(_TARGET, _day(high=high))
