...
- the featured kid's **outfit** (rain overrides temperature, then the high),
  with the kid flip-flopped between Today and Tomorrow off the date seed;
- the **temperature bar** arrow position for the day's high.
...
# Outfit-by-high cutoffs (§ Weather): < 60 °F cold, 60–72 normal, > 72 hot.
_COLD_BELOW_F = 60
_HOT_ABOVE_F = 72

# Piecewise-linear anchors mapping the day's high onto the bar as a fraction
# from the TOP (hot end). Interior anchors are the § Weather band boundaries,
...
def override_high(day: DayForecast | None, high_f: int | None) -> DayForecast | None:
    ...
    Replaces the day's high — the outfit and temperature bar re-derive from it
    — and synthesizes a clear, dry day when no forecast is available at all,
    so the subpanel can be previewed without live weather. ``None`` for
